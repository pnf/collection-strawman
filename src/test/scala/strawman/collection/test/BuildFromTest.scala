...
object BuildFrom extends BuildFromLowPriority {
  ...
  def factory[CC[_], A](orderedIterableFactory: SortedIterableFactoryWithBuilder[CC])(implicit ordering: Ordering[A]): BuildFrom.Aux[Any, A, CC[A]] =
    new BuildFrom[Any, A] {
      type To = CC[A]
  ...
  def factory[CC[X, Y] <: SortedMap[X, Y] with SortedMapOps[X, Y, CC, _], K, V](
    orderedMapFactory: SortedMapFactoryWithBuilder[CC]
  )(implicit
    ordering: Ordering[K]
  ): BuildFrom.Aux[Any, (K, V), CC[K, V]] =
    new BuildFrom[Any, (K, V)] {
